...
async def create_token(data: dict, expieres_delta: datetime.timedelta | None = None) -> str:
    to_encode = data.copy()
    if expieres_delta:
...
async def verify_token(
        token: str = Depends(oauth2_scheme)) -> bool:
    ...
    try: 
        ...
        if not token_data.exp or token_data.exp < datetime.datetime.now(datetime.timezone.utc):
